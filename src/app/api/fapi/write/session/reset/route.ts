...
import { resetFapiWritingSessionController } from "@/modules/fapi/controllers/fapiController";
...
export async function POST(request: Request) {
  return withFapiSessionCleanup(async () => {
    try {
      ...
      const result = resetFapiWritingSessionController({
      ...
      return NextResponse.json(
        { error: error instanceof Error ? error.message : "Falha ao resetar sessao de escrita da FAPI." },
        { status: 400 }
      );
    }
  });
